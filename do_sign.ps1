...
function sign ($arch, $name) { 
    ...
    if ($crosssign) {
        Invoke-CommandChecked "$arch signtool " ($signtool+"\signtool.exe") sign /fd SHA256 /v /a /s my /n ('"'+$certname+'"') /t http://timestamp.verisign.com/scripts/timestamp.dll /ac $crosssign $name 
    } else {
        Invoke-CommandChecked "$arch signtool " ($signtool+"\signtool.exe") sign /fd SHA256 /v /a /s my /n ('"'+$certname+'"') /t http://timestamp.verisign.com/scripts/timestamp.dll  $name 
    }
}
